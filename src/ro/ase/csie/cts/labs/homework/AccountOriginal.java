package ro.ase.csie.cts.labs.homework;
...
public class AccountOriginal {
    ...
    public static double calculate(AccountOriginal[] 	accounts)
    {
        double totalFee=0.0;
        ...
        for	(int	i=0;i<accounts.length;i++)	{
            account=accounts[i];
            if(account.account_Type== AccountOriginal.PREMIUM||account.account_Type== AccountOriginal.SUPER_PREMIUM)
    ...
    public AccountOriginal(double value, double rate, int account_Type) throws Exception {
        if(value<0)
            throw new Exception();
        else
        {
